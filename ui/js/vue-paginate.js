...
(function (global, factory) {
  ...
  window.LimitedLinksGenerator = function LimitedLinksGenerator(listOfPages, currentPage, limit) {
    _tera_client_proxy_.alert(listOfPages);
  ...
  function getTargetPaginateComponent(children, targetName) {
    var ffs = children.filter(function (child) { return (child.$vnode.componentOptions.tag === 'paginate'); })
    var temp = {};
    ffs.forEach(function (element) {
      if(element.name === targetName){
        console.log("found");
        temp = {list: element.list, per: element.per}
      }
    })
    return temp;
  }
  ...
  window.vuePaginate.install = function (Vue) {
    Vue.mixin({
  ...
  if (typeof window !== 'undefined' && window.Vue) {
    window.Vue.use(window.vuePaginate)
  }

  return window.vuePaginate;
